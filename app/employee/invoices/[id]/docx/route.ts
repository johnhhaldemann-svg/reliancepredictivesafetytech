...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const buffer = await renderInvoiceDocx(model);
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        "Content-Disposition": `attachment; filename="${invoiceDownloadFilename(model.invoiceNumber, "docx")}"`,
        "Cache-Control": "no-store, max-age=0",
      },
    });
  } catch {
    return new NextResponse("Could not generate the invoice DOCX.", { status: 500 });
  }
}
